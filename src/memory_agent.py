...
class MemoryAgent:
    def __init__(self, args, guiding_prompt: str = None):
        ...
        self.use_vector_similarity = getattr(self.args, 'use_vector_similarity', False)
    ...
    def update_scores(self, state_node, options_with_logits, k, r, memory_text, info=None):
        nearest_trajectories = self.cross_mem.retrieve_similar(
                ...
                r=r,
                info=info,
                use_vector=self.use_vector_similarity
            )
        ...
        # 将action_rewards中有但options_with_logits中没有的action添加到options_with_logits，并赋值normalized_prob为0
        # 只有当该action的样本数超过3个且平均奖励为正时才添加
        for action in action_rewards:
            # 检查该action是否已在options_with_logits中
            found = False
            for option_data in options_with_logits.values():
        ...
        if action_advantages:
            ...
            if positive_advs:
                # 如果有正的advantage，用最大的正值做归一化
                max_positive = max(positive_advs)
                normalized_advantages = {action: adv / max_positive for action, adv in action_advantages.items()}
            else:
                # 如果没有正的advantage，用最大负值的绝对值做归一化
                max_negative_abs = abs(min(adv_values))  # min gives most negative, abs makes it positive
...
"""
    ...
    def generate_action(self, state_node, info=None):
        ...
        # Reasoning models (o1, o3, gpt-5-mini, etc.) do not support logprobs
        _reasoning_model = any(k in self.args.llm_model.lower() for k in
                               ['o1', 'o3', 'o4', 'gpt-5-mini', 'gpt-4o-mini-high'])
        res_obj = chat_completion_with_retries(
            ...
            top_logprobs=0 if _reasoning_model else self.args.top_actions,
